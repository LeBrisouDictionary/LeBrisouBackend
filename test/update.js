...
Lab.experiment("Update", function () {
  ...
  Lab.test("Synonym already attach", function (done) {
    options = fixtures.load('update/synonym-already-attach', options)

    server.inject(options, function (response) {

      Lab.expect(response.statusCode).to.equal(20057)

      var result = response.result

      Lab.expect(result.error).to.be.equal('Bad Request')
      Lab.expect(result.message).to.be.equal('Conflict : Synonym already attached')
      setTimeout(done, delay)
  ...
  Lab.test("Relative already attach", function (done) {
    options = fixtures.load('update/relative-already-attach', options)

    server.inject(options, function (response) {

      Lab.expect(response.statusCode).to.equal(20059)

      var result = response.result

      Lab.expect(result.error).to.be.equal('Bad Request')
      Lab.expect(result.message).to.be.equal('Conflict : Relative already attached')
      setTimeout(done, delay)
  ...
  Lab.test("Antonym already attach", function (done) {
    options = fixtures.load('update/antonym-already-attach')

    server.inject(options, function (response) {

      Lab.expect(response.statusCode).to.equal(20058)

      var result = response.result

      Lab.expect(result.error).to.be.equal('Bad Request')
      Lab.expect(result.message).to.be.equal('Conflict : Antonym already attached')
      setTimeout(done, delay)
